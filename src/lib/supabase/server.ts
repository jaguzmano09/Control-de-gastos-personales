    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        setAll: (items: CookieToSet[]) => {
          try {
            items.forEach(({ name, value, options }) => cookieStore.set(name, value, options as never))
          } catch {
...
export function createServiceRoleClient() {
  ...
  return createSupabaseClient<Database, 'public', Database['public']>(url, serviceRoleKey, {

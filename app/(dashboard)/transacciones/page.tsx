...
export default async function TransaccionesPage({
  ...
  let query = supabase
    .from('transactions')
    .select(
      'id, occurred_at, type, description, amount, status, categories(name), accounts(name), wallets(name)'
    )
  ...
  return (
    <div>
      ...
      <form
        method="get"
        className="mt-6 flex flex-wrap items-end gap-3 rounded-sm border border-black/10 bg-white p-4"
      >
        <div>
          <label className="block text-xs text-ledger-muted">
            Mes y año
          </label>
          ...
          <tbody>
            {transactions.map((t) => (
              ...
              >
                ...
                <td className="py-2 pr-4 text-right text-ledger-text">
            ...
            {transactions.length === 0 && (
              <tr>
                <td
                  colSpan={8}
                  className="py-6 text-center text-ledger-muted"
                >
